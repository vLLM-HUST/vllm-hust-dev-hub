...
#ifndef __HCCL_REDUCE_ROOTINFO_TEST_H_
#define __HCCL_REDUCE_ROOTINFO_TEST_H_
...
namespace hccl
{
#define REDUCE_SUM_MAX_RANKSIZE(HcclDataType) int64_t(((int64_t(1) << int(HcclDataTypePrecision::CONCAT(HcclDataType, ))) - 1) / 2)
#define REDUCE_SUM_RESULE_OVERFLOW(rank_size,HcclDataType) bool(int64_t(rank_size) > REDUCE_SUM_MAX_RANKSIZE(HcclDataType))
class HcclOpBaseReduceTest:public HcclOpBaseTest
{
public:
    HcclOpBaseReduceTest();
    virtual ~HcclOpBaseReduceTest();
