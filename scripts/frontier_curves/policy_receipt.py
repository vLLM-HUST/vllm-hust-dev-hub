...
def receipt(before, after, *, require_exercised=True):
    ...
    exercised = all(delta[k] > 0 for k in ("calls", "admissions", "completions"))
    if require_exercised and not exercised:
        raise ValueError("Policy lifecycle not exercised")
    return dict(
        enabled=True,
        policy=POLICY,
        status="exercised" if exercised else "not-exercised",
        **delta,
