...
def capture_metrics(path):
    with urllib.request.urlopen(
        "http://127.0.0.1:33782/metrics", timeout=10
    ) as response:
...
def main(args):
    ...
    state.update(
        controller_pid=os.getpid(), parent_pid=os.getppid(), devices=[0, 1, 2, 3]
    )
    write(out / "status.json", state)
    program = getattr(args, "program", "nativepp")
    ...
    try:
        ...
        if measure:
            ...
            if metadata["serving_chips"] != 4 or metadata["serving_devices"] != [
                0,
                1,
                2,
                3,
        ...
        while True:
            ...
            try:
                with urllib.request.urlopen(
                    "http://127.0.0.1:33782/health", timeout=min(2, remaining)
                ) as response:
        ...
        command = [
            ...
            "--endpoint",
            "http://127.0.0.1:33782/v1/completions",
            "--model",
            "frontier-qwen35-pp2",
            "--tokenizer",
            str(BASE / "model"),
        ...
        write(out / "status.json", state)
        windows = (
            [("qualification", 2, 60), ("c1", 1, 900), ("c2", 2, 900), ("c8", 8, 900)]
            if measure
            else [("calibration-c4", 4, 60), ("calibration-c16", 16, 60)]
        )
        state["windows"] = []
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--program", choices=["nativepp", "pipelinepp"], default="nativepp"
    )
