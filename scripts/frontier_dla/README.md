# Matched Native, BidKV and known-budget DLA curves

The completed TP2 campaign measures Qwen3.5-35B-A3B with fixed compiled SWE
prompt lengths and output budgets, 256K context capacity, MTP2, APC, async and
graph execution. Actual generated output IDs feed subsequent turns; input
content is not fixed across runs. All three arms use the same new core capsule.
Each passed 26 retrieval probes, a 60s cache/protocol gate, then
C1/C2/C4/C8/C16 for 900s per cell. All 15 observations passed raw-stream
validation, and all arms released their devices. These are single observations,
not repeated lifecycles or an established performance gain.
...
Preparation refuses the wrong container or occupied devices, verifies the
model/workload/source files, and initially writes `software_tests_passed=false`.
`test_runtime_integration.py` exercises the real allocator for full-attention
and hybrid Mamba cache groups, the preemption controller and scheduler option.
All six actual-container integration tests passed before the campaign started.
The failed first fixture run and subsequent correction are retained below.
...
actual-runtime and hardware checks.
...
sets `software_tests_passed=true`. This runner passed in the actual target container before measurement.
...
controls that used a different core. All 15 actual phase4 observations have now
passed this importer and were imported into the website publication branch.
...
Completion receipt: the final archive SHA256 is
`192c3ce54b94aa70d28758ef09e523273afb574e099a4aa9a9c915a8ea3896f2`.
Independent final validation found all 2667 tracked source files unchanged and
all four allocated devices unowned. The owned supervisor was shut down.
DLA admission checks executed for every request but deferred none; neither
candidate selected a preemption victim. Overall decision-effect status remains
`not-exercised`, with DLA check execution recorded separately. The installed
Native/BidKV/DLA curves must not be described as demonstrated optimization gains.
