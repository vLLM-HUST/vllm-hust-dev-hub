"""Supervised manager-launched TP2 Native/Tiering qualification and measurement."""
...
def server_command(pid, program):
    ...
    while True:
        ...
        if (
            str(BASE / "phase6/.venv-r2/bin/vllm-hust-ext") in tokens
            and "run" in tokens
        ):
            return " ".join(tokens)
...
def preflight():
    if socket.gethostname() != "coder-admin-shuhao-evaluation-664b765847-wfh7z":
        raise RuntimeError("Wrong experiment container")
...
def main(args):
    ...
    try:
        ...
        while True:
            ...
            if remaining <= 0:
                raise TimeoutError("Managed TP2 health deadline")
            if not Path(f"/proc/{pid}").exists():
                raise RuntimeError("Managed TP2 exited before health")
            try:
                with urllib.request.urlopen(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--program", choices=["native", "tiering"], default="native")
